...
static void my_block_request(struct request_queue *q)
{
	...
	while (1) {
		...
		if (rq->cmd_type != REQ_TYPE_FS) {
			printk (KERN_DEBUG "Skip non-fs request\n");
		...
		printk(KERN_LOG_LEVEL
			...
			"cur_bytes=%u dir=%c, dir_val=%d\n",
			(unsigned long long) blk_rq_pos(rq), blk_rq_bytes(rq), blk_rq_cur_bytes(rq), (rq_data_dir(rq) ? 'W' : 'R'), rq_data_dir(rq));
		...
		/*	my_block_transfer(dev, sector, length, buffer, direction ) */
		my_block_transfer(dev, blk_rq_pos(rq), blk_rq_cur_bytes(rq), rq->buffer, rq_data_dir(rq));
#endif
